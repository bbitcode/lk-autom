...
async function getCachedArticles(): Promise<Article[] | null> {
...
async function scoreArticles(articles: Article[]): Promise<Article[]> {
...
async function fetchFreshArticles(): Promise<Article[]> {
  const allFeeds = [
    ...STATIC_FEEDS,
    ...KEYWORDS.map((kw) => ({
      name: `Google News`,
